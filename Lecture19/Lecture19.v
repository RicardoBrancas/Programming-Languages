...
(** Here is the single-step relation, first informally... 
            ...
            test t1 then t2 else t3 --> test t1' then t2 else t3
                      ...
                      iszro (scc v1) --> fls
                            ...
                            t1 --serrer la taille pour qu'elle soit la plus fine possible> t1'
                       ----------------------                         (ST_Iszro)
                       iszro t1 --> iszro t1'
*)
...
Example some_term_is_stuck :
  exists t, stuck t.
Proof.
  unfold stuck. exists (scc tru). split.
  - unfold normal_form. unfold not. intros. inversion H. unfold step in H0.
(** [] *)
...
(* Exercise: prove that the following term is well-typed *)
Example has_type_0:
...
Example scc_hastype_nat__hastype_nat : forall t,
  ...
  |- t \in Nat.
Proof.
  (*intros. inversion H. apply H1. Qed.*)
  (* FILL IN HERE *) Admitted.
(** [] *)
